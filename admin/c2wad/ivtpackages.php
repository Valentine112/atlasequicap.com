...
if(isset($_POST['stop'])){
  ...
  $empty = "";
    ...
    $updating = $link->prepare("UPDATE users SET activate = ?, pname = ?, pdate = ?, increase = ?, counting = ?, duration = ?, froms = ?, tos = ?, usd = ? WHERE email = ?");
    $updating->bind_param('issiiiiiis', $zero, $empty, $empty, $zero, $zero, $zero, $zero, $zero, $zero, $email);
    if($updating->execute()):
