            ...
            <th>Username</th>
			  ...
			  if(mysqli_num_rows($result) > 0){
				  while($row = mysqli_fetch_assoc($result)){  
                    ...
                    $selecting = $link->prepare("SELECT username FROM users WHERE refcode = ?");
                    $selecting->bind_param('s', $row['referred']);
                    $selecting->execute();
                    $selecting->bind_result($refUsername);
                    $selecting->fetch();
				  ...
				  ?>
						...
						<form action="users.php" method="post">
							...
							<!--<td><?php //echo $msg;?></td>-->
							<td><?= ($row['walletbalance'].$percentage); ?></td>
